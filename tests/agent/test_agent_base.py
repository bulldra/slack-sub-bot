...
def test_limit_blocks_over_max():
    ...
    assert result[:39] == blocks[:39]
    assert result[39]["type"] == "context"
    assert "省略" in result[39]["elements"][0]["text"]
...
def test_update_message_retries_on_invalid_blocks():
    from unittest.mock import MagicMock
    from slack_sdk.errors import SlackApiError
    ...
    fail_resp = {
        "ok": False,
        "error": "invalid_blocks",
        "response_metadata": {"messages": ["[ERROR] no more than 50 items allowed"]},
    }
    success_resp = {"ok": True, "ts": "12345.6789"}
    mock_slack.chat_update.side_effect = [
        SlackApiError(
            message="invalid_blocks",
            response=MagicMock(data=fail_resp, get=fail_resp.get),
        ),
    ...
    agent = AgentSlack({"channel": "C12345", "ts": "12345.6789"})
    agent._slack = mock_slack

    blocks = [{"type": "markdown", "text": "test content"}]
    agent.update_message(blocks, force=True)

    assert mock_slack.chat_update.call_count == 2
    second_call_kwargs = mock_slack.chat_update.call_args_list[1][1]
    assert second_call_kwargs["blocks"][0]["type"] == "section"
